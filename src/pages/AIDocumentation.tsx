            ...
            <div className="container mx-auto px-4 max-w-6xl py-12">
                <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <nav className="flex flex-row lg:flex-col gap-1 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0">
                            {professions.map(p => (
                                <button
                                    ...
                                    className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left transition-all whitespace-nowrap flex-shrink-0 lg:flex-shrink lg:whitespace-normal w-full text-xs font-semibold
                                        ${activeRole === p.role
                    ...
                    <main className="flex-1 min-w-0 space-y-8">
                        ...
                        {/* Header Card */}
                        <div className={`p-6 rounded-2xl border ${active.color} bg-opacity-5`}>
                            <div className="flex items-start gap-4">
                                <div className={`p-3 rounded-xl border ${active.color} bg-white dark:bg-card`}>
                                    {active.icon}
                                </div>
                                <div className="flex-1">
                                    <span className={`text-[10px] font-bold uppercase tracking-widest ${active.accent} mb-1 block`}>{active.tag}</span>
                                    <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-1">{active.role}</h2>
                                    <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">{active.headline}</p>
                                </div>
                                <Button asChild size="sm" className="rounded-xl font-bold text-xs shrink-0">
                            ...
                            <div className="space-y-4">
                                {active.prompts.map((p, i) => (
                                    <div key={i} className="rounded-xl border border-slate-200 dark:border-border overflow-hidden">
                                        <div className="px-4 py-2.5 bg-slate-50 dark:bg-card border-b border-slate-100 dark:border-border flex items-center justify-between">
                                            <span className="text-xs font-bold text-slate-700 dark:text-slate-300">{p.scenario}</span>
                                            <Button
                                                asChild
                                                variant="ghost"
                                                size="sm"
                        ...
                        {/* CTA */}
                        <div className="p-6 rounded-2xl bg-primary/5 dark:bg-primary/10 border border-primary/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            <div>
                                <p className="text-sm font-bold text-slate-900 dark:text-white mb-1">Ready to get started as a {active.role}?</p>
                                <p className="text-xs text-slate-500">Open the Studio and run your first prompt in under 60 seconds.</p>
                            </div>
                            <Button asChild className="rounded-xl font-bold uppercase tracking-wide text-xs px-6 shrink-0">
                                <Link to={`/pro/ai-studio?role=${active.role}`}>
                                    Launch {active.role} Studio <Zap className="w-3.5 h-3.5 ml-2" />
                                </Link>
                            </Button>
                        </div>
